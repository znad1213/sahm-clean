...
import { Users, ListOrdered, Briefcase, BarChart2, Settings, ShieldAlert, Loader2, FileText, Wrench } from 'lucide-react';
...
const AdminDashboardPage = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const { count: servicesDataCount, error: servicesError } = await supabase
        .from('services')
        .select('*', { count: 'exact', head: true });

      if (servicesError) throw servicesError;
      setServicesCount(servicesDataCount || 0);
  ...
  return (
    ...
    >
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <Card className="shadow-md hover:shadow-lg transition-shadow bg-card/80 backdrop-blur-sm">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">الخدمات المتاحة</CardTitle>
            <Wrench className="h-5 w-5 text-green-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{servicesCount}</div>
            <p className="text-xs text-muted-foreground">عدد الخدمات المقدمة</p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="shadow-xl bg-card/90 backdrop-blur-md">
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <Link to="/admin/manage-services">
              <Button size="lg" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-6 text-base"><Wrench className="mr-2 rtl:ml-2 rtl:mr-0 h-5 w-5"/> إدارة الخدمات</Button>
            </Link>
            <Link to="/admin/manage-content">
              <Button size="lg" className="w-full bg-accent/80 hover:bg-accent/70 text-accent-foreground py-6 text-base"><FileText className="mr-2 rtl:ml-2 rtl:mr-0 h-5 w-5"/> إدارة المحتوى</Button>
            </Link>
